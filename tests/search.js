...
asyncTest('search for one term in a verse', function testWordInVerse () {
	...
	equal( search1.countResults(), 32, 'there are 32 verses that contain "void" in kjv');
	start();
	...
	equal( search2.countResults(), 3, 'there are 3 verses that contain "cheese" in kjv');
} );
...
asyncTest('test that search is case insensitve', function testWordInVerse () {
	...
	equal( search3.countResults(), 2, 'there are 2 verses that contain "immanuel" in kjv.  Search is case insensitive.');
	start();
	...
	equal( search4.countResults(), 2, 'there are 2 verses that contain "Immanuel" in kjv.  Search is case insensitive.');
});
...
asyncTest('search for strongs number in a verse in kjv', function () {
	...
	equal( search1.countResults(), 51, 'there are 51 occurances of "H1234" in kjv');
	start();
	...
	equal( search2.countResults(), 2, 'there are 2 occurances of "H1235" in kjv');
	...
	equal( search3.countResults(), 47, 'there are 47 occurances of "H3588" in kjv, some of which are in the same verse');
});
...
asyncTest( 'search for a morphology in a verse in kjv', function () {
	...
	equal( search.countResults(), 19886, 'there are 19886 occurances of "TH8799" in kjv');
});

asyncTest( 'search for a word and strongs number in a verse in kjv', function () {
	expect(2);
	var search1 = Object.create( javascripture.api.search );
	search1.getReferences( {
		clusivity: 'inclusive',
	...
	equal( search1.countResults(), 245, 'there are 245 occurances of "H3588" or "form" in kjv');
	start();
	...
	equal( search2.countResults(), 13, 'there are 12 occurances of "H3588" and "God" in the same verse in kjv');
});
...
asyncTest( 'search for a word and strongs number in a word in kjv', function () {
	...
	equal( search1.countResults(), 1, 'there is 1 occurance of "H7363" translated "moved" in kjv' );
	start();
	...
	equal( search2.countResults(), 8, 'there are 8 occurances of "H1877" translated "grass" in kjv' );
});
/*
