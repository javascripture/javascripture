...
function loadReference(book,chapter,verse){
	...
	$.each($('#verse span'),function() {
		...
		if(lemma){
			var strongsNumberArray = lemma.replace(/strong:/gi,'').split(',');
			var root = getRoot(strongsNumberArray);
			if(root){
				$(this).addClass(root[0]);
			} else {
				$(this).addClass(strongsNumberArray[0]);
			}
		}
	});
...
var getRoot = function(strongsNumberArray) {
	var root = strongsObject[strongsNumberArray[strongsNumberArray.length - 1]];
	console.log(strongsNumberArray);
	if(root) {
		$.each(strongsNumberArray, function(index, value){
			if(root == value) {
				return root;
			} else {
				strongsNumberArray.push(value);
		setTimeout(function() {
			getRoot(strongsNumberArray);
		}, 1000);
			}
		});
	} else {
		return strongsNumberArray[0];
	}
}
...
function initializeWordPanel(spanObject) {
	var strongsNumberArray = spanObject.attr('lemma').replace(/strongs:/gi,'').split(' ');
